...
const static char *TAG = "estimator";
  ...
  if (anyEstimator != estimator) {
    if (anyEstimator == requiredEstimator) {
      ...
      if (requiredEstimator != estimator) {
        ...
        ESP_LOGI(TAG, "WARNING: Two decks require different estimators");
      }
    }
  }
...
void stateEstimatorSwitchTo(StateEstimatorType estimator) {
  ...
  //   ESP_LOGI(TAG, "Estimator type forced\n");
